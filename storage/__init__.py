...
class Storage:
    ...
    def find_task(user_id, task_id):
        ...
        change_task = db.execute('SELECT t.id, t.name, t.description, t.completed FROM users u, tasks t, userTask ut WHERE t.id = ut.idTask '
            'and u.id = ut.idUser and u.id = ? and t.id = ?', (user_id, task_id,)).fetchone()
        if change_task:
